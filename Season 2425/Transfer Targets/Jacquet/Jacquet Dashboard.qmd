...
Vizlocation <- paste0("~/Dropbox/AI lfc data/R files/AI UP/Scripts/Viz Pack/Season 2324/Transfers/Jacquet")

#set Away
Away_Team <- paste0("Rennes")
...
#set transfer target
Transfer_Target <- paste("Jérémy Jacquet")
...
#load in main data 
fileName <- paste0("~/Dropbox/AI lfc data/Data Sets/WhoScored/Transfers/Processed Games/", Match_Fixture, ".xlsx")
...
MatchSummaryData <- MatchSummaryData %>%
  left_join(PossessionTable, by = c("TeamName" = "TeamName"), copy = TRUE) %>%
  left_join(EPVTable, by = c("TeamName" = "TeamName"), copy = TRUE) %>%
  left_join(FieldTiltTable, by = c("TeamName" = "TeamName"), copy = TRUE) %>%
  mutate('EPV % Con.' = round((xG/EPV)*100,1))

MatchSummaryData <- MatchSummaryData %>%
  select('Team' = TeamName, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFFT Shots' = OFShots, 'ONT Shots' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV, 'EPV % Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success)
...
MatchSummaryHalfData <- MatchSummaryHalfData %>%
  select('Game Half' = Half, 'Team' = TeamName, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFFT Shots' = OFShots, 'ONT Shots' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success)

MatchSummaryPeriodData <- plottingData %>%
  mutate(Successful_Touch = ifelse(isTouch == "TRUE", 1, 0)) %>%
  mutate(TeamEPV = ifelse(str_detect(`outcomeType/value`, "1"), paste0(EPVvalue), 0),
         TeamEPV = as.numeric(TeamEPV)) %>%
  group_by(Game_Period, TeamName) %>% 
  summarise_at(vars(Goals, In_play_time, Touches, Final_Third_Touches, TotalShots, OFShots, OTShots, SixYard_Box_Shots, PK_Box_Shots, Out_Box_Shots, BCs, xG, NPxG, xGOT, Attempted_Passes, Successful_Passes, TeamEPV), list(sum), na.rm = TRUE) %>%
  mutate(In_play_time = round((In_play_time/60),1),
         Pass_Success = round((Successful_Passes/Attempted_Passes)*100,1),
         'EPV % Con.' = round((xG/TeamEPV)*100,1),
...
MatchSummaryPeriodData <- MatchSummaryPeriodData %>%
  select('Time Period' = Game_Period, 'Team' = TeamName, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFFT Shots' = OFShots, 'ONT Shots' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success)
...
AwaypassPlot1stWD <- plot_passnet_AI(data = plottingData,  
                                   ...
                                   plot_start = 00, plot_finish = 44, pressmap = "Yes", logo = "", passes = 2)
AwaypassPlot1stWD
```
...
AwaypassPlot2ndWD <- plot_passnet_AI(data = plottingData,  
                                   ...
                                   plot_start = 46, plot_finish = 100, pressmap = "Yes", logo = "", passes = 2)
AwaypassPlot2ndWD
```
...
AwaypassPlot1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                        ...
                                        plot_start = 00, plot_finish = firstSubAway, pitch = "", wrap = "", pressmap = "Yes", logo = "", passes = 2)
AwaypassPlot1stsubsWD

ggsave(path = Vizlocation, "Pass Map EPV 1st Sub.png", width = 2500, height = 2150, units = "px", dpi = 400)
```
...
AwaypassPlot1stAtt <- plot_passnet_AI(data = plottingData, 
                                     team_name = Away_Team, scale_stat = "EPV", 
                                     scale_color = Color_Home, text_color = Color_Home_Text, subtitle = paste0(Match_Fixture, " 1st Half"),
                                     plot_start = 00, plot_finish = 44, pitch = "Half", pressmap = "Yes", passes = 2)
AwaypassPlot1stAtt
...
AwaypassPlot2ndAtt <- plot_passnet_AI(data = plottingData,
                                     team_name = Away_Team, scale_stat = "EPV", 
                                     scale_color = Color_Home, text_color = Color_Home_Text, subtitle = paste0(Match_Fixture, " 2nd Half"),
                                     plot_start = 46, plot_finish = 100, pitch = "Half", pressmap = "Yes", passes = 2)
AwaypassPlot2ndAtt
...
AwaypassPlot1SubAtt <- plot_passnet_AI(data = plottingData,
                                      ...
                                      plot_start = 00, plot_finish = firstSubAway, pitch = "Half", pressmap = "Yes", passes = 2)
AwaypassPlot1SubAtt

#save EPV pass network
ggsave(path = Vizlocation, "Pass Map EPV 1st Sub (Attacking half passes only).png", width = 2500, height = 2150, units = "px", dpi = 400)
```
...
passing_sequencesAway <- successful_passes %>%
  mutate(
    pass_count = ifelse(Successful_Passes == 1 & TeamName == Home_Team, 1, 0),
    passing_seq = accumulate(Successful_Passes == 1 & TeamName == Home_Team, ~ifelse(.y == 0, .y, .x + .y)),
  ...
  ungroup() %>%
  select(
    minute, second, TeamName, playerId, type, `type/value`, outcome, `outcomeType/value`, pass_count, passing_seq, Pchain, PChain_pass_count, x, y, Distance_Metre, PDistance, Distance_seq, PChain_distance_count, Game_Period
  ) %>%
...
passing_sequencesTT <- successful_passes %>%
  filter(playerId == Transfer_Target) %>%
  mutate(
    pass_count = ifelse(Successful_Passes == 1 & TeamName == Away_Team, 1, 0),
    passing_seq = accumulate(Successful_Passes == 1 & TeamName == Away_Team, ~ifelse(.y == 0, .y, .x + .y)),
    Pchain = cumsum(passing_seq == 0) + 1,
    pass_count = cumsum(pass_count)
  ) %>%
  group_by(Pchain) %>%
...
MatchSummaryPLayerData <- MatchSummaryPLayerData %>%
  select(Team = TeamName, role, Name = playerId, '% of Team Possession' = Possession, `% of Team On Ball Time` = On_Ball_Time, Touches = Touches, 'Total Shots' = TotalShots, 'Out Box Shots' = Out_Box_Shots, BCs, Goals, xG, xGOT,  'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, Assists, 'Key Passes' = Key_Passes, EPV = TeamEPV, '% of Team EPV' = Team_EPV_P, 'Aerials Total' = Aerial_total, 'Aerials Won' = Aerial_Won, 'Aerial Success %' = Aerial_Success, 'Duels Total' = Ground_Duels_Total, 'Duels Won' = Ground_Duels_Won, 'Duel Success' = Duel_Success)



Player_Summary_plot_data <- MatchSummaryPLayerProfileData %>%
  select(Team = TeamName, Name = playerId, role, '% Possession' = Possession, `% On Ball Time` = On_Ball_Time, Touches = Touches, 'Total Shots' = TotalShots, 'OFT Shots', 'OT Shots', 'Out Box Shots' = Out_Box_Shots, BCs, Goals, xG, xGOT, '% xG' = Team_xG_P, Touches, PK_Box_Touches, 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, Assists, 'Key Passes' = Key_Passes, EPV = TeamEPV, '% EPV' = Team_EPV_P, 'Aerials Total' = Aerial_total, 'Aerials Won' = Aerial_Won, 'Aerial Success %' = Aerial_Success, 'Duels Total' = Ground_Duels_Total, 'Duels Won' = Ground_Duels_Won, 'Duel Success %' = Duel_Success, '% Duels' = Team_Duels_P, '% Passes' = Team_Passes_P, '% Shots' = Team_Shots_P, '% Aerials' = Team_Aerial_P)
...
PassConnections <- PassConnections %>%
  filter(type == "Pass") %>% 
  select(Passer = playerId, Receiver = receiver, Passer_Team = TeamName, Receiver_Team = receiver_team, Successful_Passes, BCs, Key_Passes) 
...
AwayconvexPlot1stSubs <- TouchPlayerPlot_AI(TouchDataTTarget,  title = Away_Team,
...
AwaytouchPlot1Sub <- TouchTeamPlot_OPTA(TTTeamTouchData,  title = paste0(Away_Team, " Team On Ball Touch"),
...
AwayEPVHeatMapplotFT <- AwayEPVHeatMapDataFT %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play EPV Heat Map - Full Game"), 
...
#save EPV pass network
ggsave(path = Vizlocation, "EPV heat map.png", width = 2500, height = 2150, units = "px", dpi = 400)

```

```{r}
#| height: 100%
EPVHeatMapplotFT <- AwayEPVHeatMapDataFT %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#454545") +
  theme_pitch() +
  geom_tile() +
  ...
  #plot heat map
  stat_density_2d_filled(aes(#color = after_stat(level), 
    fill = after_stat(level)), geom = "polygon", alpha = 0.4, adjust = 0.5, linewidth = 0.5) +
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play EPV Heat Map - Full Game"), 
                            subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, EPV, Box figures = % of Total EPV \nDirection of Attack Left to Right"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
#set time period
EPVHeatMapData1sub <- AwayEPVHeatMapDataFT %>%
...
#Post first subs map
#set time period
EPVHeatMapDataP1sub <- AwayEPVHeatMapDataFT %>%
  filter(expandedMinute > firstSubAway)
...
AwayEPVHeatMapplot1sub <- AwayEPVHeatMapData1sub %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play EPV Heat Map - Up to 1st Subs"), 
                     subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, EPV, Box figures = % of Total EPV \nDirection of Attack Left to Right"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
EPVHeatMapplot1sub <- EPVHeatMapData1sub %>%
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play EPV Heat Map - Up to first subs"), 
                     subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, EPV, Box figures = % of Total EPV \nDirection of Attack Left to Right"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
#first half EPV Mao
#set time period
EPVHeatMapData1H <- AwayEPVHeatMapData %>%
  filter(period == 1)
...
#Second half EPV Map
#set time period
EPVHeatMapData2H <- AwayEPVHeatMapData %>%
  filter(period == 2)
...
AwayEPVHeatMapplot1H <- AwayEPVHeatMapData1H %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play EPV Heat Map - First Half"), 
                     subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, EPV, Box figures = % of Total EPV \nDirection of Attack Left to Right"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
AwayEPVHeatMapplot2H <- AwayEPVHeatMapData2H %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play EPV Heat Map - Second Half"), 
                     subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, EPV, Box figures = % of Total EPV \nDirection of Attack Left to Right"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
EPVHeatMapplot1H <- EPVHeatMapData1H %>%
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play EPV Heat Map - First Half"), 
                     subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, EPV, Box figures = % of Total EPV \nDirection of Attack Left to Right"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
EPVHeatMapplot2H <- EPVHeatMapData2H %>%
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play EPV Heat Map - Second Half"), 
                     subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, EPV, Box figures = % of Total EPV \nDirection of Attack Left to Right"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
PPDAMapplotFT <- ggplot(average_PPDAFT, aes(x = zone_x, y = zone_y, fill = avg_PPDA)) +
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play Defensive Actions Heat Map - Full Game"), 
...
AwayPPDAHeatMapplotFT <- AwayPPDAHeatMapDataFT %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play Defensive Actions Heat Map - Full Game"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher Defensive Actions (DA), \nDirection of Attack Right to Left"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
#save EPV pass network
ggsave(path = Vizlocation, "PPDA heat map.png", width = 2500, height = 2150, units = "px", dpi = 400)
...
AwayPPDAHeatMapplot1sub <- AwayPPDAHeatMapData1sub %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play Defensive Actions Heat Map - Up to 1st Subs"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher Defensive Actions (DA), \nDirection of Attack Right to Left"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
HomePPDAHeatMapplot1sub <- HomePPDAHeatMapData1sub %>%
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play Defensive Actions Heat Map - Up to first subs"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher Defensive Actions (DA), \nBox figures = % of Total DA, \nDirection of Attack Right to Left"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
AwayPPDAHeatMapplot1H <- AwayPPDAHeatMapData1H %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play Defensive Actions Heat Map - First Half"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher Defensive Actions (DA), \nDirection of Attack Right to Left"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
AwayPPDAHeatMapplot2H <- AwayPPDAHeatMapData2H %>%
  ...
  #formatting
  labs(title = paste(Transfer_Target," Open Play Defensive Actions Heat Map - Second Half"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher Defensive Actions (DA), \nDirection of Attack Right to Left"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
HomePPDAHeatMapplot1H <- HomePPDAHeatMapData1H %>%
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play Defensive Actions Heat Map - First Half"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher Defensive Actions (DA), \nBox figures = % of Total DA, \nDirection of Attack Right to Left"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
HomePPDAHeatMapplot2H <- HomePPDAHeatMapData2H %>%
  ...
  #formatting
  labs(title = paste0(Match_Away," Open Play Defensive Actions Heat Map - Second Half"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher Defensive Actions (DA), \nBox figures = % of Total DA, \nDirection of Attack Right to Left"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
HometouchPlot1Sub <- TouchTeamPlot_OPTA(HomePPDAHeatMapData,  title = paste0(Match_Away, " Team Off Ball Touch"),
...
dashboard_data <- dashboard_data %>%
  ...
  mutate(Goal_point_X = as.numeric(Goal_point_X)) %>%
  mutate(Goal_point_Y = as.numeric(Goal_point_Y)) %>%
  select(X = x, Y = y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, TeamName, TotalShots, xG, NPxG, xGOT, minute, expandedMinute, result = type, player = playerId)
...
#  geom_text(aes(x = 45, y = max(TotalxG), label = "HT"), colour = "black", vjust = -1, size = 4) +
  ...
  labs(x = "Match Time (mins)", 
       y = "xG, NPxG, xGOT",
       title = paste0(Away_Team, " ", xGlineTitle_text), 
       caption = My_caption,
       subtitle = paste0(Match_Fixture))  +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        panel.background = element_blank(),
...
xGDensityPlot <- dashboard_data %>%
  ...
  stat_density_2d(aes(#color = after_stat(level), 
    fill = after_stat(level), text = paste("xG: ", ..level..)), geom = "polygon", alpha = 0.4, linewidth = 1) +
...
#save EPV pass network
ggsave(path = Vizlocation, "xG Density heat map.png", width = 2500, height = 2150, units = "px", dpi = 400)
...
```{r away shot plot}
#|height: 110%
Away_S_plot <- plot_shot_AI(Awaydashboard_data, highlight_goals = TRUE, average_location = FALSE, mapType = AwayHA,  mapName = Home_Team, teamColor = Color_Home, matchDate = Game_date, matchFixture = Match_Fixture, dataSource = "Opta")
...
#|height: 110%
Away_S_plotW <- plot_shot_AI(Awaydashboard_data, highlight_goals = TRUE, average_location = FALSE, mapType = AwayHA,  mapName = Home_Team, teamColor = Color_Home, text_color = Color_Home_Text, matchDate = Game_date, matchFixture = Match_Fixture, dataSource = "Opta", wrap = "Player")
...
#|height: 110%
Away_S_plotWT <- plot_shot_AI(Awaydashboard_data, highlight_goals = TRUE, average_location = FALSE, mapType = AwayHA,  mapName = Home_Team, teamColor = Color_Home, text_color = Color_Home_Text, matchDate = Game_date, matchFixture = Match_Fixture, dataSource = "Opta", wrap = "Time")
